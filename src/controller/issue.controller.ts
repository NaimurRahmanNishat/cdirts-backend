...
export const getIssueById = catchAsync(async (req: Request, res: Response) => {
  const { issueId } = req.params;
  if (!issueId) return res.status(400).json({ success: false, message: "Issue ID is required" });
...
export const editIssue = catchAsync(async (req: AuthRequest, res: Response) => {
  ...
  let issueImages = images;
  if (image && !images) issueImages = [image];
  if (!issueImages || !Array.isArray(issueImages)) issueImages = [];

  const updatedIssue = await Issue.findByIdAndUpdate(
    issueId,
    { title, category, description, images: issueImages, location, division, date },
